...
export default function AppFrame() {
  const [initialMobile, setInitialMobile] = useState<boolean | null>(null); // set once, decides which layout gets UnicornScene
  const [isMobile, setIsMobile] = useState<boolean | null>(null); // tracks current viewport
  ...
  // Detect mobile on mount (once for initial, live for current)
  useEffect(() => {
  ...
  useEffect(() => {
    if (initialMobile === null || initialMobile) return;
    const t = setTimeout(() => setWindowed(true), cfg.autoPlayDelay * 1000);
    return () => clearTimeout(t);
  }, [initialMobile]); // eslint-disable-line react-hooks/exhaustive-deps

  // Whether the current layout can use UnicornScene (matches initial load)
  const hasAnimatedBg = initialMobile === isMobile;
  ...
  // Wait for detection before rendering
  if (isMobile === null) {
  ...
  if (isMobile) {
    return (
      <div className="relative overflow-hidden flex flex-col bg-amber-100">
        {hasAnimatedBg && (
          <div className="absolute inset-0 w-full h-full z-0 pointer-events-none [&_*]:!pointer-events-none">
            <UnicornScene
        ...
        <div className="flex flex-col items-center justify-center gap-6 px-6 pt-24 pb-28 z-10 relative">
      ...
      {/* Popcorn popping up from bottom */}
    ...
    >
      ...
      >
        ...
        <div className="flex-1 flex flex-col relative min-w-0 min-h-full overflow-hidden bg-amber-100 z-0">
          {hasAnimatedBg && <div className="absolute inset-0 w-full h-full -z-10 pointer-events-none [&_*]:!pointer-events-none">
            <UnicornScene
              projectId="Dy4crfooAOZD6FlsT7Xl"
              width="100%"
              height="100%"
